...
def test_dwell_times_for_instance(df_instance, dwell_range):
    ...
    for dwell in dwell_range:
        reward_value = compute_reward_for_dwell_time(
            df_instance, dwell, alpha=1.0, beta=0.5, gamma=0.2
        )
        rows.append({"DstIP": dstip, "DwellTime": dwell, "Reward": reward_value})
        logger.info(
            f"Dwell time: {dwell:.1f} minutes, Computed Reward: {reward_value:.3f}"
        )
    optimal_row = max(rows, key=lambda row: row["Reward"])
    optimal_dwell = optimal_row["DwellTime"]
    optimal_reward = optimal_row["Reward"]
    logger.info(
        f"Optimal dwell time for this instance: {optimal_dwell:.1f} minutes with reward {optimal_reward:.3f}\n"
    )
...
def main():
    ...
    with open(csv_filename, "w", newline="") as csvfile:
        ...
        for dst, group_df in groups:
            rows, optimal_dwell, optimal_reward, initial_state = (
