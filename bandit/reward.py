...
def compute_reward_for_dwell_time(
    df,
    dwell_time,
    alpha=1.0,
    beta=1.0,
    ...
    if dwell_time < threshold:
        ...
        sustained_bonus = (
            bonus_factor * (len(second_half) / len(first_half))
            if len(first_half) > 0
            else 0
        )
        extra_penalty = 0
    ...
    reward = (
        alpha * weighted_unique_ips
        + beta * weighted_session_count
        + sustained_bonus
        - gamma * duration_penalty
    )
    return reward
